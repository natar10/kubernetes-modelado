...
RabbitMQ & Message Broker & Cola de mensajes para coordinación asíncrona \\
...
RabbitMQ & 3.12-management & Message Broker asíncrono & Apache Kafka, Redis Streams \\
...
\subsubsection{RabbitMQ como Message Broker}
\textbf{RabbitMQ} fue elegido para la coordinación asíncrona de mensajes entre microservicios:
\begin{itemize}
    \item Implementa el protocolo AMQP, estándar para mensajería empresarial
    \item Garantiza el orden de entrega de mensajes dentro de una cola (FIFO)
    \item Proporciona persistencia de mensajes, evitando pérdidas ante fallos
    \item Incluye una interfaz de administración web (Management UI) para monitorización
    \item Imagen Docker ligera disponible (\texttt{rabbitmq:3.12-management-alpine})
\end{itemize}

Es utilizado por \texttt{eventos-service} y \texttt{participacion-service} para procesar inscripciones en orden de llegada (requisito RQ-24), desacoplando las escrituras críticas del flujo síncrono de peticiones HTTP.

\subsection{Dependencias entre Componentes}

Tilt gestiona las dependencias mediante \texttt{resource\_deps}, asegurando que los componentes se inicien en el orden correcto: Bases de Datos $\rightarrow$ RabbitMQ / Keycloak $\rightarrow$ Microservicios $\rightarrow$ API Gateway $\rightarrow$ Cliente.
...
\midrule
\multicolumn{4}{l}{\textit{Mensajería}} \\
rabbitmq & 5672 & rabbitmq.eventos-system.svc & AMQP \\
rabbitmq & 15672 & rabbitmq.eventos-system.svc & HTTP (Management) \\
\midrule
...
http://localhost:15672 & rabbitmq:15672 & RabbitMQ Management UI \\
\bottomrule
\end{tabular}
...
\begin{itemize}
    ...
    \item \texttt{eventos-service} y \texttt{participacion-service} se conectan a RabbitMQ (puerto 5672) para la coordinación asíncrona de inscripciones, garantizando el orden de llegada.
\end{itemize}
...
\subsection{RabbitMQ - Message Broker}

\textbf{RabbitMQ} proporciona mensajería asíncrona entre los microservicios del sistema.
Es utilizado por \texttt{eventos-service} y \texttt{participacion-service} para procesar
las inscripciones a eventos en orden de llegada (requisito RQ-24).
Link al manifiesto completo:
\href{https://github.com/natar10/kubernetes-modelado/blob/main/k8s/services/rabbitmq.yaml}{rabbitmq.yaml}

\begin{lstlisting}[caption={rabbitmq.yaml - Configuración de RabbitMQ (extracto)}, label={lst:rabbitmq}]
apiVersion: apps/v1
kind: Deployment
metadata:
  name: rabbitmq
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{Images/k8s/rabbit.png}
    \caption{Interfaz RabbitMQ Management UI}
    \label{fig:rmq}
\end{figure}
...
# Mensajeria
k8s_yaml('services/rabbitmq.yaml')
...
k8s_resource('eventos-service',
    labels=['services'],
    resource_deps=['db-eventos', 'rabbitmq'],
    port_forwards=['8082:8080'])

k8s_resource('participacion-service',
    labels=['services'],
    resource_deps=['db-participacion',
      'eventos-service', 'rabbitmq'],
    port_forwards=['8083:8080'])

k8s_resource('api-gateway',
    labels=['gateway'],
    resource_deps=['usuarios-service',
      'eventos-service', 'participacion-service'],
